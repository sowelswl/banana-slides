...
Enable **Quality Control** from **Settings → Image Generation Configuration** or from the switch below the generation button on the Slide Preview page.

When enabled, each generated image is not saved as a version immediately. A vision model first checks whether:
- the slide contains garbled, unreadable, or clearly incorrect text
- the illustration, layout, or rendering quality is too low
- the visual content, style, or layout substantially deviates from the page description and prompt

If the review rejects the image, Banana Slides automatically regenerates it, up to 3 attempts. Only images that pass the review are saved to the database and added to version history. If all attempts fail, the page is marked as failed and prompts you to adjust the page description or prompt.
